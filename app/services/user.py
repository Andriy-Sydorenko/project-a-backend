...
class UserService:
    ...
    async def get_user(
        ...
        user = await self.user_repo.get_user(field, value, selected_fields, include_relations)
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")
        return user

    async def create_user(
        self, email: str, password: str, username: Optional[str] = None, selected_fields: Optional[list[str]] = None
    ) -> User:
